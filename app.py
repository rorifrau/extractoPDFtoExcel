...
class ExtractorExtractoBancario:
    ...
    def extraer_operaciones_fraccionadas(self, texto: str) -> List[Dict]:
        ...
        # Buscar operaciones fraccionadas línea por línea
        i = 0
        while i < len(lineas):
            linea = lineas[i].strip()
            
            # Buscar líneas que contengan fecha y operaciones fraccionadas (B.B.V.A. o CAJ.LA CAIXA)
            if re.search(r'^\d{2}\.\d{2}\.\d{4}.*(B\.B\.V\.A\.|CAJ\.LA CAIXA)', linea):
                try:
                    # Extraer datos de la línea principal
                    partes = linea.split()
                    fecha = partes[0]
                    ...
                    patron_numero = r'^\d+[,\.]\d{2}$'
                    
                    for parte in partes[1:]:
                        if re.match(patron_numero, parte):
                            try:
                                numeros.append(float(parte.replace(',', '.')))
                            except ValueError:
                                continue
                        elif parte not in ['B.B.V.A.', 'CAJ.LA', 'CAIXA', 'OF.7102']:
                    ...
                    for j in range(i+1, min(i+6, len(lineas))):
                        ...
                        # Buscar plazo - diferentes formatos
                        plazo_match = re.search(r'Plazo\s+(\d+\s+De\s+\d+)', linea_siguiente, re.IGNORECASE)
                        if not plazo_match:
                            plazo_match = re.search(r'PRÓXIMO PLAZO\s+(\d{2}-\d{2}-\d{4})', linea_siguiente, re.IGNORECASE)
                        if plazo_match:
                            plazo = plazo_match.group(1)
                        
                        # Buscar "Importe pendiente después"
                        if "Importe pendiente después" in linea_siguiente:
                            # El importe está en la misma línea o en la siguiente
                    ...
                    # Crear la operación si tenemos suficientes datos
                    if len(numeros) >= 1:  # Al menos necesitamos el importe de operación
                        operacion = {
                            'fecha': fecha,
                            'concepto': concepto,
                        ...
                        if st.session_state.get('debug_mode', False):
                            st.write(f"✅ Operación fraccionada encontrada: {fecha} - {concepto} - {numeros} - Plazo: {plazo}")
        ...
        # Método alternativo: buscar en sección específica si no encontramos nada
        if not operaciones:
            if st.session_state.get('debug_mode', False):
                st.write("🔄 No se encontraron operaciones con método principal, probando método alternativo...")
            
            # Buscar sección de operaciones fraccionadas - diferentes formatos
            patrones_seccion = [
                r'IMPORTE OPERACIONES FRACCIONADAS.*?OPERACIÓN.*?CONCEPTO.*?OPERACIÓN.*?PENDIENTE.*?AMORTIZADO.*?INTERESES.*?MENSUAL(.*?)(?=TOTAL OPERACIONES FRACCIONADAS|OPERACIONES DE LA TARJETA|Página|\Z)',
                r'IMPORTE OPERACIONES FRACCIONADAS(.*?)(?=TOTAL OPERACIONES FRACCIONADAS|OPERACIONES DE LA TARJETA|Página|\Z)'
            ]
            
            for patron_seccion in patrones_seccion:
                match_seccion = re.search(patron_seccion, texto, re.DOTALL | re.IGNORECASE)
                
                if match_seccion:
                    seccion_texto = match_seccion.group(1) if match_seccion.lastindex else match_seccion.group(0)
                    
                    if st.session_state.get('debug_mode', False):
                        st.text_area("🔍 Sección alternativa encontrada", seccion_texto[:1000], height=150)
                    
                    # Procesar líneas de esta sección
                    lineas_seccion = seccion_texto.split('\n')
                    for linea in lineas_seccion:
                        linea = linea.strip()
                        if re.search(r'^\d{2}\.\d{2}\.\d{4}', linea) and ('B.B.V.A.' in linea or 'CAJ.LA CAIXA' in linea):
                            # Procesar igual que arriba
                            partes = linea.split()
                            if len(partes) >= 3:  # fecha + concepto + al menos 1 número
                                try:
                                    fecha = partes[0]
                                    patron_numero = r'^\d+[,\.]\d{2}$'
                                    numeros = [float(p.replace(',', '.')) for p in partes if re.match(patron_numero, p)]
                                    ...
                                    if 'B.B.V.A.' in linea:
                                        ...
                                        concepto_partes = [p for p in partes[1:] if not re.match(patron_numero, p)]
                                    ...
                                    if len(numeros) >= 1:
                                        ...
                                        if st.session_state.get('debug_mode', False):
                                            st.write(f"✅ Operación alternativa encontrada: {fecha} - {concepto}")
                                except ValueError:
                                    continue
                    
                    if operaciones:  # Si encontramos algo, salir del bucle
                        break
                    ...
                    if re.match(r'^\d{2}\.\d{2}\.\d{4}', linea):
                        ...
                        if len(partes) >= 4:
                            try:
                                ...
                                patron_numero = r'^\d+[,\.]\d{2}$'
                                importe_candidatos = [p for p in partes if re.match(patron_numero, p)]
...
def main():
    st.title("📊 Convertidor de Extractos Bancarios PDF a Excel v1.7")
    st.markdown("---")
    ...
    with st.expander("ℹ️ Información de la aplicación"):
        ...
        **Versión 1.7** - Corregidos errores de sintaxis y mejorado soporte multi-formato
        """)
    ...
    st.markdown(
        ...
        Convertidor de Extractos Bancarios v1.7 | Desarrollado para Streamlit por ROF
        </div>
        """, 
