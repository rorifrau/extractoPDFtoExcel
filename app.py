...
if __name__ == "__main__":
    main()
        ...
        if not operaciones:
            ...
            # Buscar: fecha + CAJ.LA CAIXA + OF.XXXX + números + plazo
            patron_continuo = r'(\d{2}\.\d{2}\.\d{4})\s*(CAJ\.LA\s*CAIXA|COMERCIAL\s*MAYORARTE)\s*(?:OF\.\d{4})?\s*(?:INNOV)?\s*(\d+[,\.]\d{2})\s*(\d+[,\.]\d{2})\s*(\d+[,\.]\d{2})\s*(\d+[,\.]\d{2})\s*(\d+[,\.]\d{2})\s*(?:Plazo\s*(\d+\s*De\s*\d+)|PRÓXIMO\s*PLAZO\s*(\d{2}-\d{2}-\d{4}))?'
            
            matches = re.finditer(patron_continuo, texto, re.IGNORECASE | re.DOTALL)
            
            for match in matches:
                try:
                    fecha = match.group(1)
                    concepto = match.group(2).replace(' ', ' ').strip()  # Normalizar espacios
                    ...
                    # Extraer plazo
                    plazo = ""
                    if match.group(8):  # Plazo X De Y
                        plazo = match.group(8)
                    elif match.group(9):  # PRÓXIMO PLAZO fecha
                        plazo = match.group(9)
                    
                    # Buscar importe pendiente después en el texto cercano
                    importe_pendiente_despues = 0.0
        ...
        if not operaciones:
            ...
            # Buscar todas las fechas seguidas de conceptos conocidos
            patrones_especificos = [
                ...
                r'(\d{2}\.\d{2}\.\d{2})\s+B\.B\.V\.A\.\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})'
            ]
            ...
            for patron in patrones_especificos:
                ...
                for match in matches:
                    try:
                        ...
                        # Determinar concepto basado en el patrón
...
def main():
    ...
    st.markdown(
        """
        <div style='text-align: center; color: #666; font-size: 0.8em;'>
        Convertidor de Extractos Bancarios v1.8 | Desarrollado para Streamlit por ROF
        </div>
        """, 
        unsafe_allow_html=True
    )
