...
class ExtractorExtractoBancario:
    ...
    def extraer_operaciones_fraccionadas(self, texto: str) -> List[Dict]:
        ...
        while i < len(lineas):
            ...
            # Buscar líneas que contengan fecha y B.B.V.A.
            if re.search(r'^\d{2}\.\d{2}\.\d{4}.*B\.B\.V\.A\.', linea):
                try:
                    ...
                    if len(numeros) >= 4:  # Necesitamos al menos 4 números
                        operacion = {
                            'fecha': fecha,
                            'concepto': concepto if concepto else 'B.B.V.A.',
                        ...
                        if st.session_state.get('debug_mode', False):
                            st.write(f"✅ Operación fraccionada encontrada: {fecha} - {concepto} - {numeros}")
        ...
        if not operaciones:
            ...
            # Buscar sección entre headers específicos
            patron_seccion = r'IMPORTE OPERACIONES FRACCIONADAS.*?OPERACIÓN.*?CONCEPTO.*?OPERACIÓN.*?PENDIENTE.*?AMORTIZADO.*?INTERESES.*?MENSUAL(.*?)(?=TOTAL OPERACIONES FRACCIONADAS|OPERACIONES DE LA TARJETA|Página|\Z)'
            ...
            if match_seccion:
                ...
                for linea in lineas_seccion:
                    ...
                    if re.search(r'^\d{2}\.\d{2}\.\d{4}', linea):
                        ...
                        partes = linea.split()
                        if len(partes) >= 6:  # fecha + concepto + 4 números mínimo
                            try:
                                fecha = partes[0]
                                numeros = [float(p.replace(',', '.')) for p in partes if re.match(r'^\d+[,\.]\d{2}$', p)]
                                concepto_partes = [p for p in partes[1:] if not re.match(r'^\d+[,\.]\d{2}$', p)]
