...
def main():
    st.title("📊 Convertidor de Extractos Bancarios PDF a Excel v1.5")
    st.markdown("---")
    ...
    with st.expander("ℹ️ Información de la aplicación"):
        ...
        **Versión 1.5** - Mejorado procesamiento de operaciones fraccionadas
        """)
    ...
    st.markdown(
        ...
        Convertidor de Extractos Bancarios v1.5 | Desarrollado para Streamlit por ROF
        </div>
        """, 
    ...
    main(), parte):
        ...
        if not operaciones:
            ...
            patrones_seccion = [
                ...
                r'13\.12\.2021.*?CAJ\.LA CAIXA.*?OF\.7102.*?50,00.*?50,00.*?0,00.*?0,00.*?0,00'
            ]
                    ...
                    for linea in lineas_seccion:
                        ...
                        if re.search(r'^\d{2}\.\d{2}\.\d{4}', linea) and ('B.B.V.A.' in linea or 'CAJ.LA CAIXA' in linea):
                            ...
                            if len(partes) >= 3:  # fecha + concepto + al menos 1 número
                                try:
                                    fecha = partes[0]
                                    numeros = [float(p.replace(',', '.')) for p in partes if re.match(r'^\d+[,\.]\d{2}
...
if __name__ == "__main__":
    main(), p)]
                                    ...
                                    if 'B.B.V.A.' in linea:
                                        ...
                                        concepto_partes = [p for p in partes[1:] if not re.match(r'^\d+[,\.]\d{2}
